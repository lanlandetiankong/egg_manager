...
public class UserAccountXlsServiceImpl extends MyXlsBaseServiceImpl implements UserAccountXlsService {
    ...
    @Override
    public void dealCheckExportSingleWithTemplate2Web(UserAccountEntity loginUser, HttpServletResponse response, DefineMenuEntity defineMenuEntity, AntdFileUploadBean fileUploadBean, Long[] checkIds) throws Exception {
    ...
    @Override
    public void dealAllExportSingleWithTemplate2Web(UserAccountEntity loginUser, HttpServletResponse response, DefineMenuEntity defineMenuEntity, AntdFileUploadBean fileUploadBean) throws Exception {
        //导出文件名
        String excelFileName = defineMenuEntity.getMenuName() + "选择导出表";
        String excelPath = uploadProps.getLocationPrefix() + fileUploadBean.getUrlLocation();
