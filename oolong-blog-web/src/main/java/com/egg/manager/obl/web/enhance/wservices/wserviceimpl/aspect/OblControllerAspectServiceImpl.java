package com.egg.manager.obl.web.enhance.wservices.wserviceimpl.aspect;
...
public class OblControllerAspectServiceImpl implements OblControllerAspectService {
    ...
    protected <T extends MyBaseWebLogMgo> T dealSetValToBaseLogMgo(T logMgo, JoinPoint joinPoint, HttpServletRequest request) {
        try {
            ...
            if (method != null) {
                ...
                OblPcWebQueryLog oblPcWebQueryLog = method.getAnnotation(OblPcWebQueryLog.class);
                if (oblPcWebQueryLog != null) {
                    clazzInfoLog.setAction(this.gainApiOperationMsgWhenBlank(method, oblPcWebQueryLog.action()));
                    clazzInfoLog.setLogDescription(this.gainApiOperationNotesIfBlank(method, oblPcWebQueryLog.description()));
                    //请求的全路径(代码中取得)
                    logMgo.setFullPath(oblPcWebQueryLog.fullPath());
                }
            }
            if (request != null) {
                //获取UserAgent相关信息
     ...
     */
    @Override
    public void dealSetValToQueryLog(OblPcWebQueryLogMgo logMgo, JoinPoint joinPoint, HttpServletRequest request, OblPcWebQueryLog queryLogAnno) {
        try {
     ...
     */
    @Override
    public void dealSetValToOperationLog(OblPcWebOperationLogMgo logMgo, JoinPoint joinPoint, HttpServletRequest request, OblPcWebOperationLog operationLogAnno) {
        try {
            logMgo = this.dealSetValToBaseLogMgo(logMgo, joinPoint, request);
     ...
     */
    @Override
    public void dealSetValToLoginLog(OblPcWebLoginLogMgo logMgo, JoinPoint joinPoint, HttpServletRequest request, OblPcWebLoginLog loginLogAnno) {
        try {
            logMgo = this.dealSetValToBaseLogMgo(logMgo, joinPoint, request);
